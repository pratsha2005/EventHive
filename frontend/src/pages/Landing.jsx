    ...
    const getEvents = async () => {
      try {
        ...
        if (userData?.role === "organizer") {
          ...
          response = await axios.get(`${backendUrl}/api/events/getAllEventByManagerId`, { withCredentials: true });
        } else {
          // Fetch all events for regular users
          response = await axios.get(`${backendUrl}/api/user/getAllEvents`, { withCredentials: true });
        }
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex justify-between items-center mb-12">
            <h2 className="text-3xl font-bold text-gray-900">
              {sectionTitle} <span className="text-blue-600">{userData?.role !== "organizer" ? "" : ""}</span>
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {visibleEvents.map((event) => (
              <div key={event._id} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition duration-300">
                <img
                ...
                <div className="p-6">
                  ...
                  <div className="text-sm text-gray-500 mb-1">📍 {event.location?.venue || "Online"}</div>
                  <div className="text-sm text-gray-500">🕒 {new Date(event.startDateTime).toLocaleDateString()}</div>
                </div>
              </div>
            ))}
          </div>
